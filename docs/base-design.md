...
3. 任务结束后必须在执行日志末尾追加一条精简里程碑记录，包含时间、任务/WP、关键修改或结论、最小验证证据、未验证项/风险、遗留决定、Git 状态和建议 Commit 信息。没有代码修改也要记录原因与验证结果。
4. 默认只追加，不覆盖历史结论；但维护者明确要求压缩日志时，可以把重复的逐条回显整理为当前权威摘要，并在摘要中保留关键失败根因、修复、最终证据和未验证边界。压缩后后续仍只追加精简记录；历史记录与当前代码冲突时，继续追加更正，不得静默掩盖冲突。
5. 日志必须区分静态检查、服务端运行验证、真实客户端验证、跨 shard 验证和未验证内容；不能用其中一种验证替代另一种，也不能把计划目标写成已完成事实。重复的逐命令回显、相同结果的多次轮询和无新结论的等待不再记录。
...
9. 客户端只开放观战相机和观战 UI；物品栏、装备栏、鼠标携带物品和制作栏必须隐藏并禁用；
10. 服务端拒绝 Spectator 的动作、物品和玩法 RPC。

上述保护是运行时 guard，不是状态快照。任何保护项都必须成对撤销，并保持幂等。

物品界面保护不能只依赖右键入口或客户端隐藏。服务端必须拒绝 Spectator 的装备、卸下、拖拽转移、丢弃、拾取、使用和制作请求；客户端隐藏物品栏、装备栏、鼠标物品和制作栏只是对应的交互层收口。所有绕过右键的鼠标拖拽路径也必须经过同一权威拒绝规则。即使物品已经因旧状态、竞态或绕过路径进入装备栏，装备物品的主动和被动效果也必须在 Spectator 期间失效，包括发光、拾取、攻击、防御、光环、速度、抗性和其他 gameplay 效果；退出观战后再按原状态恢复。
