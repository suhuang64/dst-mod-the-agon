...
### 2026-09-05：WP10——公平覆盖首轮绑定修复

- 根因/修改：真实 Wilson/女武神首次绑定返回 `INVALID_PLAYER_STATE_ADAPTER`；逐适配器管线定位为严格环境读取未声明全局 `TechTree`，已按官方 `builder.lua` 显式 `require("techtree")`。
- 验证/边界：失败实例已销毁；修复后专服重新加载成功并回到 `CORE_READY`。客户端因重启需重新加入，公平字段、死亡复活、掉线重连和离场恢复尚未开始验收。
